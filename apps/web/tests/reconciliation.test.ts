...
testIntegration("reconciliation endpoint returns provider fields", async () => {
  const transferId = await createTransfer();
  const res = await fetchJson(
    `${INTEGRATION_BASE}/api/transfers/${transferId}/reconciliation`,
    { headers: DEV_HEADERS }
  );
  assert.equal(res.status, 200);
  const payload = res.json as { transferId?: string; providerPayoutId?: string | null };
  assert.equal(payload.transferId, transferId);
  assert.equal("providerPayoutId" in payload, true);
});
