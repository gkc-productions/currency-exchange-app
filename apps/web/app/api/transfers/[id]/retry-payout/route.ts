...
export async function POST(
  ...
  if (transfer.status === TransferStatus.COMPLETED) {
    const response: RetryResponse = { ok: true, status: transfer.status };
    return NextResponse.json(response);
  }

  if (transfer.status !== TransferStatus.FAILED) {
    return NextResponse.json({ error: "Retry only allowed after failure." }, { status: 400 });
  }

  const updated = await prisma.transfer.updateMany({
    where: { id: transfer.id, userId: user.id, status: TransferStatus.FAILED },
    data: {
  ...
  if (updated.count === 1) {
    await prisma.transferEvent.create({
      data: {
        ...
        message:
          typeof transfer.payoutAttemptCount === "number"
            ? `Payout retry requested. attempt=${transfer.payoutAttemptCount}`
  ...
  const response: RetryResponse = {
    ok: true,
    status: updated.count === 1 ? TransferStatus.READY : transfer.status,
  };
  return NextResponse.json(response);
}
